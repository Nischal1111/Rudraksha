...
const Navbar = () => {
    ...
    return (
        <>
            <div className='shadow-sm top-0 fixed flex flex-col items-center justify-between px-4 md:px-8 py-2 bg-white w-full z-[9999]'>
                <div className='w-full flex justify-between items-center font-medium uppercase text-sm tracking-wide'>
                    ...
                    <div className='hidden md:flex items-center gap-8'>
                        <Button className='bg-primary' size='md' onPress={() => setSearchOpen(!searchOpen)} isIconOnly>
                    ...
                    <div className='flex md:hidden items-center gap-4'>
                        {isLoggedIn && 
                            <div className='flex items-center relative'>
                                ...
                                <Link href={"/checkout"}>
                                    <Button variant='light' isIconOnly className='bg-transparent text-base text-primary p-0' startContent={<CiShoppingCart size={24} className='text-[#E8B86D]' />}></Button>
                                </Link>
                            </div>
                        }
            ...
            {/* Mobile Sidebar */}
            <div className={`fixed top-0 right-0 h-full w-[280px] bg-white shadow-xl z-[10000] transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}>
                <div className='flex flex-col h-full'>
                    ...
                    <div className='flex justify-between items-center p-4 border-b'>
                        <div className='flex items-center gap-2'>
                            ...
                            <h1 className={`${stylishFont.className} text-xl leading-5`}>Khandbari</h1>
                        </div>
                        <Button 
                    ...
                    {isLoggedIn ? (
                        <div className='p-4 border-b'>
                            ...
                            <div className='mt-3 flex flex-col gap-2'>
                                <Link href="/profile">
                                    <p className='text-sm text-gray-600 hover:text-primary'>Profile</p>
                                </Link>
                                <Link href="/my-orders">
                                    <p className='text-sm text-gray-600 hover:text-primary'>My Orders</p>
                                </Link>
                                <p 
                        ...
                        <div className='p-4 border-b'>
                            <div className='flex flex-col gap-3'>
                                <Button size='sm' className='text-white rounded-md bg-primary w-full' onPress={() => handleLogin()}>
                                    Login
                                </Button>
                                <Button variant='light' className='text-primary border border-primary rounded-md w-full' onPress={() => handleSign()}>
                                    Signup
                                </Button>
                            </div>
                        </div>
                            ...
                            {nav.map(item => (
                                <li key={item.title}>
                                    <Link href={item.link!} onClick={() => setSidebarOpen(false)}>
                                        <div className={`p-2 rounded-md ${isActive(item.link!) ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
                                            <p className='font-normal text-sm uppercase tracking-wider'>{item.title}</p>
                                        </div>
                                    </Link>
                                </li>
                            ))}
                            ...
                            <li>
                                ...
                                >
                                    <div className='flex justify-between items-center'>
                                        <p className='font-normal text-sm uppercase tracking-wider'>More</p>
                                        <IoIosArrowDown className={`transition-transform duration-300 ${mobileMoreOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                </div>
                                ...
                                {mobileMoreOpen && (
                                    <ul className='ml-4 mt-2 flex flex-col gap-2'>
                                        {dropdownItems.map((item) => (
                                            <li key={item.title}>
                                                <Link href={item.link} onClick={() => setSidebarOpen(false)}>
                                                    <p className='text-sm text-gray-500 p-2 hover:text-primary'>{item.title}</p>
                                                </Link>
                                            </li>
                                        ))}
                                    </ul>
